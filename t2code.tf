...
provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/drlraj2805/hctask2.git /var/www/html",
    ]
 }
}
...
resource "aws_s3_bucket" "my_bucket" {
  ...
  provisioner "local-exec" {
    when = destroy 
    command = "echo y | rmdir /s hctask2"
  }
}
...
// Provide github repo link here after gitclone to provide your webserver code.
    command = "git clone https://github.com/drlraj2805/hctask2.git"
  } 
}
...
// Provide path here according To your system's file system
  source = "C:/Users/A/Desktop/hybrid/task2/pic.jpg"
  acl    = "public-read"
} 
  ...
  provisioner "remote-exec" {
      inline = [ 
        # sudo su << \"EOF\" \n echo \"<img src='${aws_cloudfront_distribution.s3_distribution.domain_name}'>\" >> /var/www/html/code.html \n \"EOF\"
            "sudo su << EOF",
         "echo \"<img src='http://${aws_cloudfront_distribution.s3_distribution.domain_name}/${aws_s3_bucket_object.object1.key}'>\" >> /var/www/html/code.html",
          "EOF"
     ]
  }
}
...
resource "null_resource" "link" {
  ...
  provisioner "local-exec" {
    command = "echo 'http://${aws_instance.ins1.public_ip}/code.html' > Link_To_Webpage.txt"
  } 
} 